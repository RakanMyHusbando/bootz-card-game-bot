import { ChatInputCommandInteraction, Collection, Embed, EmbedBuilder, Events, time } from 'discord.js';
        ...
        if (timestamps.has(interaction.user.id) && (now < (timestamps.get(interaction.user.id) || 0) + cooldownAmount)) 
            return interaction.reply({ embeds: [new EmbedBuilder()
                ...
                .setDescription(`Please wait ${
                    ((((timestamps.get(interaction.user.id) || 0)+ cooldownAmount)-now)/1000).toFixed(1)
            } more seconds before reusing the "${command.name}" command.`)], ephemeral: true })
